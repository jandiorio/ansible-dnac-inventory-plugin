...
class InventoryModule(BaseInventoryPlugin):
    ...
    def _login(self):
        ...
        try:
            ...
            raise AnsibleError('failed to login to DNA Center: {0}'.format(e))
    ...
    def _get_inventory(self):
        ...
        inventory_url = 'https://{0}/dna/intent/api/v1/network-device'.format(self.host)
    ...
    def _get_sites(self):
        ...
        for site in sites:
            ...
            if (site['locationType'] == 'building'):
                site_dict.update(
                    {
                        'name': 'bld_{0}'.format(normalized_site_name),
                        'id': site['id'],
        ...
        for site in self._site_list:
            if site['parentId'] in site_ids:
                ...
                try:
                    ...
                    raise AnsibleParserError('adding child sites failed:  {0} \n {1}:{2}'.format(e, site['name'], parent_name))
            elif self.toplevel:
                try:
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        try:
            ...
            raise AnsibleParserError('getting options failed:  {0}'.format(e))

        # Attempt login to DNAC
        login_results = self._login()
        if login_results.status_code not in [200,201,202,203]: 
            raise AnsibleError('failed to login: {0}'.format(login_results.status_code))
